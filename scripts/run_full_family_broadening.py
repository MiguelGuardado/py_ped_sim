...
"""
#ex: python run_ped_sim -t run_full_family_broadening.py -c ipumps_sibship_dist.txt -y 1850 1860 1870
...
if __name__ == '__main__':
    ...
    join_cmd = f'python run_single_family_broadening.py -n1 main_family.nx -n2 fam0.nx -p1 main_family_profiles.txt -p2 fam0_profiles.txt -o {u_output} -cf {founders[0]}'
    ...
    while errorcode.returncode == 1:
        ...
        if user_args.main_family:
            ...
            join_cmd = f'python run_single_family_broadening.py -n1 {mf}.nx -n2 fam0.nx -p1 {mf}_profiles.txt -p2 fam0_profiles.txt -o {u_output} -cf {founders[0]}'
        else:
            join_cmd = f'python run_single_family_broadening.py -n1 main_family.nx -n2 fam0.nx -p1 main_family_profiles.txt -p2 fam0_profiles.txt -o {u_output} -cf {founders[0]}'
    ...
    for i in founders[1:]:
        ...
        subprocess.run(sim_ped_cmd, shell=True)
        print(count)
        join_cmd = f'python run_single_family_broadening.py -n1 {u_output}.nx -n2 fam{count}.nx -p1 {u_output}_profiles.txt -p2 fam{count}_profiles.txt -o {u_output} -cf {i}'
        errorcode = subprocess.run(join_cmd, shell=True, capture_output=True)
        while errorcode.returncode == 1:
